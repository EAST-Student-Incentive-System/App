...
def initialize():
    ...
    event2 = create_active_event(jane.id, 'Tech Talk', 'Seminar', 'Learn about the latest trends in technology with industry experts.', 'Engineering Building, Room 101')
    event3 = create_active_event(jane.id, 'Yoga Class', 'Social', 'Relax and unwind with a yoga session led by a certified instructor.', 'Campus Gym')
    event4 = create_active_event(jane.id, 'Cultural Festival', 'Workshop', 'Experience the rich diversity of our campus with food, music, and performances from around the world.', 'Student Center Lawn')
    event5 = create_active_event(jane.id, 'Career Fair', 'Social', 'Connect with potential employers and learn about internship and job opportunities.', 'Exhibition Hall')
    event6 = create_active_event(jane.id, 'Art Workshop', 'Workshop', 'Get creative with a hands-on art workshop led by local artists.', 'Art Building, Room 202')
    event7 = create_active_event(jane.id, 'Environmental Cleanup', 'Volunteery', 'Join us for a campus cleanup event and earn points while making a positive impact on the environment.', 'Meet at Campus Entrance')
    event8 = create_active_event(jane.id, 'Cooking Class', 'Social', 'Learn to cook a new dish with a cooking class led by a local chef.', 'Culinary Arts Building, Room 303')
    ...
    event10 = create_active_event(jane.id, 'Sports Tournament', 'Social', 'Compete in a friendly sports tournament with your fellow students. All skill levels welcome!', 'Campus Sports Field')
    event11 = create_active_event(jane.id, 'Movie Night', 'Social', 'Join us for a movie night under the stars with free popcorn and drinks.', 'Campus Lawn')
    event12 = create_active_event(jane.id, 'Volunteer Fair', 'Volunteery', 'Learn about volunteer opportunities on and off campus and sign up to make a difference in your community.', 'Student Center Lobby')
    event13 = create_active_event(jane.id, 'Hackathon', 'Seminar', 'Collaborate with fellow students to develop innovative solutions to real-world problems in our 24-hour hackathon.', 'Engineering Building, Room 101')
    event14_start = now
